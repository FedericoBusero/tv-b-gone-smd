...
Modified for Attiny85 01/02/2018
...
In this version the POWER codes are selected by a macro:

#define US_CODES 1

to use the US codes or

#define EU_CODES 1

to use the EU codes. This means the a pin is left!
...
extern const IrCode* const PowerCodes[] PROGMEM;
...
int main(void) {
  ...
  for(i=0 ; i < j; i++) {   
    ...
    code_ptr = (PGM_P)pgm_read_word(code_ptr);
...
void tvbgone_sleep( void )
{
  ...
  PORTB |= _BV(IRLED);     // turn off IR LED
  PORTB &= ~_BV(LED);   // turn off visible 
...
void quickflashLEDx( uint8_t x ) {
  while( x > 0 ) {
     PORTB |= _BV(LED);    // turn off visible LED at PB0 by pulling pin to +3V
     delay_ten_us(10000);     // 150 millisec delay between flahes
     PORTB &= ~_BV(LED);   // turn on visible LED at PB0 by pulling pin to ground
     delay_ten_us(10000);     // 150 millisec delay between flahes
     x-= 1;
  }
